...
public class CustumerService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String email1) throws UsernameNotFoundException {
		...
		if (user == null) {
			System.out.println("Ko tìm thấy tài khoản" + email1);
			throw new UsernameNotFoundException(project.green.shop.helper.AppString.usernameNotFound);
		}
		System.out.println("Tìm thấy tài khoản tại Service " + user.getEmail());
		return new project.green.shop.security.MyUserDetails(user);
	}

	/*
	 * @Transactional public UserDetails loadUserByIdToken(Long) throws
	 * UsernameNotFoundException { Custumer user = cusrepo.getByEmail(email1); if
	 * (user == null) { System.out.println("Ko tìm thấy tài khoản"); throw new
	 * UsernameNotFoundException(project.green.shop.helper.AppString.
	 * usernameNotFound); } System.out.println(" " + user.getEmail()); return new
	 * project.green.shop.security.MyUserDetails(user); }
	 */
	...
	public Custumer getById(Integer id) throws Exception {
		if(cusrepo.getById(id)==null)
		{
